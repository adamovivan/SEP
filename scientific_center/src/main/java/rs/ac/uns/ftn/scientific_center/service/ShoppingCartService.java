...
public class ShoppingCartService {
	...
	public ShoppingCart addMagazine(Long magazineId) throws NullPointerException {
		PricelistItem pricelistItemMagazine = pricelistItemRepository
				.findByMembershipMagazineIdAndMembershipSubscriptionType(magazineId, SubscriptionType.READER_PAYS);
		...
		while (pricelistItemIterator.hasNext()) {
			...
			if (!plItem.getId().equals(pricelistItemMagazine.getId()) && plItem.getMembership().getMagazine() != null) { // different
																															// magazine
				needToClear = true;
				break;
			}
			// If articles originates from different magazine
			else if (plItem.getMembership().getArticle() != null
					&& !plItem.getMembership().getArticle().getMagazine().getId().equals(magazineId)) {
				needToClear = true;
				break;
			}
		}
	...
	public ShoppingCart addArticleReader(Long articleId) throws NullPointerException {
		...
		while (pricelistItemIterator.hasNext()) {
			...
			if (plItem.getMembership().getArticle() != null) {
				...
				if (!pricelistItemArticle.getMembership().getArticle().getMagazine().getId()
						.equals(plItem.getMembership().getArticle().getMagazine().getId())) {
					needToClear = true;
					break;
				}
			} // magazine
				...
				if (!pricelistItemArticle.getMembership().getArticle().getMagazine().getId()
						.equals(plItem.getMembership().getMagazine().getId())) {
					needToClear = true;
					break;
				}
			}
	...
	public ShoppingCart addArticleAuthor(Long articleId) throws NullPointerException {
		Article article = articleRepository.findById(articleId)
				.orElseThrow(() -> new NullPointerException("Article doesn't exist."));
		PricelistItem pricelistItem = pricelistItemRepository.findByMembershipMagazineIdAndMembershipSubscriptionType(
				article.getMagazine().getId(), SubscriptionType.OPEN_ACCESS);
		...
		rs.ac.uns.ftn.scientific_center.model.User user = userRepository.findByUsername(
	...
	public PaymentOrderResponse pay() {
		...
		return restTemplate.postForObject(paymentUrl,
				new PaymentOrderRequest(totalPrice, email, transaction.getOrderId(), paymentCallbackUrl),
	...
	private void changeArticleMembership(ShoppingCart shoppingCart) {
		...
		while (itemIterator.hasNext()) {
			...
			List<OpenAccessRequest> oars = openAccessRequestRepository.findByMembershipIdAndAuthorUsernameAndActive(
					membership.getId(),
